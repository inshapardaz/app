...
const post = (url, contents, language = 'en', contentType = 'application/json') => {
  if (contents) {
    delete contents.links;
  }

  return fetchWrapper.post(url, contents, {
    Accept: 'application/json', 'Content-Type': contentType, 'Accept-Language': language, 'Content-Language': language,
  })
    .then((data) => parseObject(data));
};
...
const put = (url, contents, contentType = 'application/json', language = 'en') => {
  ...
  return fetchWrapper.put(url, contents, {
    Accept: 'application/json', 'Content-Type': contentType, 'Accept-Language': language, 'Content-Language': language,
  })
    .then((data) => parseObject(data));
};
...
export default {
  ...
  getArticle: (libraryId, periodicalId, volumeNumber, issueNumber, sequenceNumber) => get(`${librariesUrl()}/${libraryId}/periodicals/${periodicalId}/volumes/${volumeNumber}/issues/${issueNumber}/articles/${sequenceNumber}`),
  getArticleContents: (libraryId, periodicalId, volumeNumber, issueNumber, sequenceNumber, language) => get(`${librariesUrl()}/${libraryId}/periodicals/${periodicalId}/volumes/${volumeNumber}/issues/${issueNumber}/articles/${sequenceNumber}/contents?language=${language}`),
  addArticleContents: (url, language, newContent) => post(`${url}?language=${language}`, newContent),
  updateArticleContents: (url, language, newContent) => put(`${url}?language=${language}`, newContent),

  setArticleSequence: (articles) => post(articles.links.article_sequence, articles.data.map((a) => ({ id: a.id, sequenceNumber: a.sequenceNumber }))),
